...
function Panel({ title, aside, children }: { title: string; aside: string; children: ReactNode }) {
  return <section className="rounded-xl border border-border bg-surface p-4 md:p-5"><div className="mb-3 flex items-baseline justify-between gap-3"><h2 className="text-sm font-medium tracking-tight">{title}</h2><span className="font-mono text-[11px] text-muted">{aside}</span></div>{children}</section>;
}
function Empty({ children }: { children: ReactNode }) { return <p className="text-sm text-muted">{children}</p>; }
function Err({ children }: { children: ReactNode }) { return <div role="alert" className="rounded-lg border border-danger/40 bg-sunken px-3 py-2 text-sm text-danger">{children}</div>; }
import {
