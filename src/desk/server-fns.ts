...
export const postAlpacaCancelAll = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .handler(async ({ context }) => {
    const { postAlpacaCancelAllImpl } = await import("./server-fns-impl.server");
    return postAlpacaCancelAllImpl(context.userId);
  });
...
export const runIntelligentCycle = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .handler(async ({ context }) => {
    const { runIntelligentCycleImpl } = await import("./server-fns-impl.server");
    return runIntelligentCycleImpl(context.userId);
  });

export const fetchSleeveScheduler = createServerFn({ method: "GET" })
  .middleware([authMiddleware])
  .handler(async ({ context }) => {
    const { fetchSleeveSchedulerImpl } = await import("./server-fns-impl.server");
    return fetchSleeveSchedulerImpl(context.userId);
  });

export const postSleeveScheduler = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .validator(z.object({ enabled: z.boolean() }))
  .handler(async ({ context, data }) => {
    const { postSleeveSchedulerImpl } = await import("./server-fns-impl.server");
    return postSleeveSchedulerImpl(context.userId, data.enabled);
  });
