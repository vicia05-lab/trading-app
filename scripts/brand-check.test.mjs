...
test("SKILL.md and AGENTS.md name the marker path and bound this script uses", () => {
  ...
  for (const rel of [".grok/skills/og/SKILL.md", "AGENTS.md"]) {
    // Grok Builder injects skills; GitHub does not contain those platform files.
    if (rel.startsWith(".grok/") && !existsSync(join(TEMPLATE_ROOT, rel))) continue;
...
test("the sections that own the brand-task prohibition never affirm a wait", () => {
  ...
  for (const section of PROHIBITION_SECTIONS) {
    if (section.rel.startsWith(".grok/") && !existsSync(join(TEMPLATE_ROOT, section.rel))) continue;
...
test("SKILL.md tells the pass to self-check with the flag this CLI accepts", {
  skip: !existsSync(join(TEMPLATE_ROOT, ".grok/skills/og/SKILL.md")) && "Grok-injected skill is not part of the GitHub repository",
}, () => {
